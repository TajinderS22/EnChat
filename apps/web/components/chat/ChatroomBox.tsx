...
const ChatroomBox = ({ chatRoom, newMessageChatId }: ChatRoomBoxType) => {
  ...
  return (
    <div
      className={`flex border-b bg-teal-200/20 ${id == ActiveChatRoom && " bg-teal-400/60 "} rounded-lg 
     m-2 p-2 h-18
    ...
    >
      ...
      {newMessageChatId == id && (
        <div className="w-2 h-2 mr-4 bg-green-400 animate-ping rounded-full"></div>
      )}
    </div>
  );
};
