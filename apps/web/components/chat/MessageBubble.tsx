...
const MessageBubble = (props:MessageBubbleType) => {
  ...
  return (
    <div className={`w-full flex  mx-auto ${Number(User.id)==userId?" justify-end ":" justify-start "}" `}>
        <div className={`max-w-4/12  px-4 pr-6 ${Number(User.id)==userId? " bg-[#2ab6aa] text-[#212121] ":" bg-[#cccccc] text-[#212121]"} p-2 overflow-scroll m-2 rounded-lg`}>
            {decryptedMessage}
        </div>
    </div>
  )
